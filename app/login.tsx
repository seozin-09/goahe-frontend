...
export default function LoginScreen() {
  ...
  const onLogin = async () => {
    ...
    if (!trimmedNick || !trimmedEmail || !trimmedPw) {
      Alert.alert("로그인", "닉네임, 이메일, 비밀번호를 입력해줘.");
      return;
    }

    // (선택) 이메일 형식 간단 체크
    if (!trimmedEmail.includes("@") || !trimmedEmail.includes(".")) {
      Alert.alert("로그인", "이메일 형식이 올바르지 않아.");
      return;
    }

    try {
      // ✅ 로그인 성공했다고 가정하고 저장 (백엔드 붙이면 여기만 교체)
      const user = {
      ...
      await AsyncStorage.setItem("auth_token", "temp_token");

      router.replace("/(tabs)/write");
    } catch (e) {
      Alert.alert("로그인", "저장 중 오류가 발생했어.");
    }
  };
  ...
  return (
    <View style={[styles.root, { backgroundColor: EDGE }]}>
      ...
      <View
        ...
        style={[
          styles.bottomFill,
          { height: insets.bottom, backgroundColor: EDGE },
        ]}
      />
